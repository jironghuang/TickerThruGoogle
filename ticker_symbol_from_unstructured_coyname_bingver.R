...
ticker_symbol = function(unstruc_name){
  ...
  struc_name = paste("https://www.bing.com/search?q=",struc_name,"+morningstar",sep = "")
  html = getURL(struc_name, ssl.verifypeer = FALSE)
  
  if(grepl('financial statements, historical, balance sheet',html)){
    ticker = str_locate(html,'financial statements, historical, balance sheet')[1,1]
    ticker = substring(html, first = ticker - 50, last = ticker+200)  
